"""Flask implementation of the BuzzBot Web API.

Replaces the previous Starlette/Uvicorn server so that the CLI `--api` mode
now runs a Flask app. Endpoints are the same for compatibility.

Run (dev):
  PYTHONPATH=src python3 src/main.py --api --api-port 8000 --api-reload
"""
...
TEMPLATE_DIR = BASE_DIR / 'webui' / 'templates'
